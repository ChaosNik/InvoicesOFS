...
function format_money_pdf($money, $currency = null)
{
    ...
    $symbolText = $currency->swap_currency_symbol ? trim($currency->symbol) : $currency->symbol;
    $symbol = '<span style="font-family: DejaVu Sans;">'.$symbolText.'</span>';

    $currency_with_symbol = '';
    if ($currency->swap_currency_symbol) {
        $currency_with_symbol = $format_money.' '.$symbol;
    } else {
        $currency_with_symbol = $symbol.$format_money;
    }
